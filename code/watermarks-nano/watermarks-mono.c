...
static void ProcessAudioBlock(const int16_t *samples16K);
static void PerformSampleRateConversion(int16_t *samples8K, const int16_t *samples16K, int16_t *samples24K);
static void DecodeCB(const int16_t *samples);
static void DecodeN2(const int16_t *samples);
static void DecodeN6(const int16_t *samples);
...
int main()
{
    ...
    int16_t frames[AUDIO_BLOCK_16K];
    while (fread(frames, sizeof(int16_t), AUDIO_BLOCK_16K, stdin) == (size_t)AUDIO_BLOCK_16K)
    {
...
void ProcessAudioBlock(const int16_t *samples16K)
{
    int16_t samples8K[AUDIO_BLOCK_8K];
    int16_t samples24K[AUDIO_BLOCK_24K];

    PerformSampleRateConversion(samples8K, samples16K, samples24K);
    DecodeCB(samples8K);
    DecodeN2(samples24K);
    DecodeN6(samples16K);
}

void PerformSampleRateConversion(int16_t *samples8K, const int16_t *samples16K, int16_t *samples24K)
{
    static struct {
...
void DecodeCB(const int16_t *samples)
{
    static void *context;
    ...
    static int16_t buffer[FFT_SIZE];
    ...
    for (int i = 0; i != AUDIO_BLOCK_8K; ++i)
    {
        ...
        if (++index == FFT_SIZE)
        {
            ...
            ALTFIXFFT_Fix16CalcFft2048(buffer, temp, START_POSITION, END_POSITION);
            ...
            memmove(buffer, buffer + OVERLAP_PTS, (sizeof(int16_t) * (FFT_SIZE - OVERLAP_PTS)));
            index -= OVERLAP_PTS;
        }
    }
}
...
void DecodeN6(const int16_t *samples)
{
    static struct {
        N6_Sfft_Flt_ControlBlock_t fft;
        N6_A_ControlBlock_t dec;
    } context;

    if (context.fft == NULL)
    {
        context.fft = malloc(N6_Sfft_Flt_Bytes_In_Control_Block());
        context.dec = malloc(N6_A_Bytes_In_Control_Block());
        N6_Sfft_Flt_Init(context.fft);
    ...
    while (blocks--)
    {
        N6_Sfft_Flt_Discard_t discard;
        N6_Sfft_FftBinPowers_t powers;
        N6_Sfft_Flt_Program_Mono(samples, discard, powers, context.fft);
        N6_A_Program_Decode(powers, context.dec);
        N6_Sfft_Flt_Commercial_Mono(discard, powers, context.fft);
        N6_A_Commercial_Decode(powers, context.dec);
        samples += N6_SFFT_NUM_SAMPLES;
    }
}
